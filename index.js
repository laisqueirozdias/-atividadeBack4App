...
const renderizaLista = (lista, tarefas) => {
  ...
  tarefas.forEach((atividade) => {
      ...
      inputCheck.onclick = () => updateTask(atividade);
      inputCheck.type = "checkbox";
      inputCheck.checked = atividade.done;
      const itemText = document.createTextNode(
          `${atividade.description} `
      );
      const buttonDelete = document.createElement("button");
      buttonDelete.innerHTML = "X";
      buttonDelete.onclick = ()=> deleteTask(atividade.objectId);
      const listItem = document.createElement("li");
      listItem.className = atividade.done ? "done" : "";
